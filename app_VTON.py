...
parser.add_argument(
    "--share", type=str, default=False, help="Set to True to share the app publicly."
)
parser.add_argument(
    "--lowvram", action="store_true", help="Enable CPU offload for model operations."
)
parser.add_argument(
    "--load_mode",
    default=None,
...
parser.add_argument(
    "--fixed_vae", action="store_true", default=True, help="Use fixed vae for FP16."
)
...
def auto_crop_upload(editor_value, crop_flag):
    ...
    try:
        ...
        if crop_flag:
            ...
            print(
                "auto_crop_upload: Cropping done. Crop region:",
                left,
                top,
                right,
                bottom,
...
def start_tryon(
    ...
    if is_checked_crop:
        if ORIGINAL_IMAGE is not None:
            orig = ORIGINAL_IMAGE
            print(
                "start_tryon: Using ORIGINAL_IMAGE from global with resolution:",
                orig.size,
            )
        else:
            orig = human_img_orig
            print(
                "start_tryon: GLOBAL ORIGINAL_IMAGE not found. Using auto-cropped human image with resolution:",
                orig.size,
            )
        ...
        print(
            f"start_tryon: Computed crop region on original image: left_orig: {left_orig}, top_orig: {top_orig}, target_width: {target_width}, target_height: {target_height}"
        )
        print(
    ...
    if is_checked:
        ...
        if (
            dict.get("layers")
            and len(dict["layers"]) > 0
            and dict["layers"][0] is not None
        ):
            ...
            print(
                "start_tryon: Manual mask binary mask:",
                type(mask),
                mask.mode,
                mask.size,
            )
...
with image_blocks as demo:
    gr.Markdown(
        "## V17 - IDM-VTON 👕👔👚 improved by SECourses : 1-Click Installers Latest Version On : https://www.patreon.com/posts/122718239"
    )
    gr.Markdown(
    ...
    with gr.Row():
        with gr.Column():
            ...
            imgs.upload(
                ...
                inputs=[
                    imgs,
                    gr.Checkbox(value=True, label="Use auto-crop & resizing"),
                ],
                outputs=imgs,
            )
            with gr.Row():
            ...
            with gr.Row():
                is_checked_crop = gr.Checkbox(
                    label="Yes", info="Use auto-crop & resizing", value=True
                )
            example = gr.Examples(
                inputs=imgs, examples_per_page=2, examples=human_ex_list
            )
        with gr.Column():
        ...
        with gr.Column():
            ...
            with gr.Row():
                ...
                is_randomize_seed = gr.Checkbox(
                    label="Randomize seed for each generated image", value=True
                )
                number_of_images = gr.Number(
                    label="Number Of Images To Generate (it will start from your input seed and increment by 1)",
                    minimum=1,
                    maximum=9999,
                    value=1,
                    step=1,
